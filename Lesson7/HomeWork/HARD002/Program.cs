...
int[,] ShuffleArray(int[,] array, int rows, int cols)
    {
        int[,] shuffledArray = new int[rows, cols];
        Array.Copy(array, shuffledArray, rows * cols);
        ...
        for (int k = 0; k < iterations; k++)
        {
            ...
            do
            {
                row1 = random.Next(rows);
                column1 = random.Next(cols);
                row2 = random.Next(rows);
                column2 = random.Next(cols);
            }
            while (shuffledArray[row1, column1] == shuffledArray[row2, column2]);
            int temp = shuffledArray[row1, column1];
            shuffledArray[row1, column1] = shuffledArray[row2, column2];
            shuffledArray[row2, column2] = temp;
        }
...
PrintArray(array);
System.Console.WriteLine();
int[,] shuffledArray = ShuffleArray(array, rows, cols);
